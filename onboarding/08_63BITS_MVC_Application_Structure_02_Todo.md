...
![Sample rendered Product Detail page showing a page title ("Samsung 75 Inch QLED 4K Smart TV"), a product image, SKU, name, price ($1700.00), and a description paragraph](../images/08_63BITS_MVC_Application_Structure_02_Todo/image1.png)
...
![ProductWebsiteModel.cs showing public class ProductWebsiteModel : WebsiteModelBase with a ProductDTO Product property and a GetViewModel method, annotated to note it has no async Task](../images/08_63BITS_MVC_Application_Structure_02_Todo/image2.png)
...
![RouteKeys63.cs showing the static class with a public const string ProductID = "productID" property, in Solution Explorer under Domain/Utilities](../images/08_63BITS_MVC_Application_Structure_02_Todo/image3.png)
...
![ProductWebsiteController.cs showing the Product() action implemented: it calls var viewModel = Model.GetViewModel() and returns View(_viewName, viewModel)](../images/08_63BITS_MVC_Application_Structure_02_Todo/image6.png)
...
![ProductWebsiteView.cshtml with the model directive @model ProductWebsiteModel.ViewModel declared at the top of the file, shown alongside Solution Explorer with ProductWebsiteView.cshtml selected under Views/Website/Products](../images/08_63BITS_MVC_Application_Structure_02_Todo/image7.png)
...
![File Explorer and Solution Explorer showing product.html located alongside blog_post.html, blog_posts.html, and products.html in wwwroot/html](../images/08_63BITS_MVC_Application_Structure_02_Todo/image8.png)

Open `product.html` using your web browser. This is how your final result must look.

![Sample static product.html rendered in the browser: the Samsung 75 Inch QLED 4K Smart TV page with SKU, title, price, and description text next to the product image](../images/08_63BITS_MVC_Application_Structure_02_Todo/image9.png)

Navigate to **SixtyThreeBits.Web → wwwroot → html**, open **product.html**, and copy the static HTML.

![product.html opened in Visual Studio, showing the <section class="py-5"> block containing the product image, SKU, title, price, and description markup, annotated "Copy static HTML"](../images/08_63BITS_MVC_Application_Structure_02_Todo/image10.png)

Paste it into your view and replace the static content with `ViewModel` properties.

![ProductWebsiteView.cshtml after pasting the section markup, with @Html.Raw(Model.ProductCoverImageHttpPath), @Html.Raw(Model.PageTitle), and @Html.Raw(Model.ProductPrice) replacing the static image src, title, and price values](../images/08_63BITS_MVC_Application_Structure_02_Todo/image11.png)
...
![Apple iPhone 15 Pro Max product page running in the browser at localhost/products/1, showing the product image, SKU, title, price ($1199), and description](../images/08_63BITS_MVC_Application_Structure_02_Todo/image12.png)
...
![Products page running in the browser showing four product cards (iMac 24" Apple M3, iPad Pro 12.9, Apple Watch Series 9, Apple iPhone 15 Pro Max), with an arrow pointing to the View Details button under the iMac card](../images/08_63BITS_MVC_Application_Structure_02_Todo/image14.png)

Upon click, the product detail page will be loaded.

![iMac 24" Apple M3 product detail page running in the browser, showing the product image, SKU, title, price ($2699), and description](../images/08_63BITS_MVC_Application_Structure_02_Todo/image15.png)
